...
def openrouter_descriptions(candidates):
    ...
    return {item["repository"]: item for item in parsed.get("items", []) if item.get("keep") is True and item.get("repository") and item.get("summary")}
...
def build_public_items(candidates, descriptions, old_stars):
    ...
    for repo in candidates:
        ...
        items.append({"full_name": full_name, "name": full_name.replace("/", " / "), "description": ai.get("summary", fallback)[:240], "category": ai.get("category", repo.get("language") or "Open Source"), "language": repo.get("language") or "Other", "color": LANGUAGE_COLORS.get(repo.get("language"), "#77736b"), "gained": f"+{trend_stars:,}", "stars": stars, "forks": repo.get("forks_count", 0), "url": repo.get("html_url"), "updated_at": repo.get("pushed_at")})
    return items
...
def write_outputs(items, now):
    ...
    for index, item in enumerate(items, 1):
        lines.extend([f"## {index}. [{item['full_name']}]({item['url']})", f"**{item['category']} · {item['language']}**", "", item["description"], "", f"Stars: {item['stars']:,} · 오늘 증가: {item['gained']}", ""])
